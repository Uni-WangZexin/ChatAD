...
def worker_llama_cpp(
    input_queue, output_list, gpu_id, batch_size, model_path=MODEL_PATH
):
...
def worker_vllm(input_queue, output_list, gpu_id, batch_size, model_path=MODEL_PATH):
    ...
    try:
        ...
        llm = LLM(
            ...
            tensor_parallel_size=gpu_per_model,
    ...
    try:
        ...
        pipe = pipeline(
            ...
            backend_config=TurbomindEngineConfig(
                tp=gpu_per_model, session_len=ctx_length * 2
            ),
        )
    ...
    for gpu_id in range(0, num_gpus, gpu_per_model):
        ...
        if engine == "llama":
            p = multiprocessing.Process(
                target=worker_llama_cpp,
                args=(input_queue, output_list, gpu_id_str, batch_size, model_path),
            )
        elif engine == "vllm":
            p = multiprocessing.Process(
                target=worker_vllm,
...
def escape_newlines_in_quotes(json_string):
    matches = list(
        re.finditer(r'(?<!\\)"([^"\\]*(?:\\.[^"\\]*)*)"', json_string, re.DOTALL)
    )
    fixed_json = []
    last_end = 0
...
if __name__ == "__main__":
    # Load questions
    batch_questions = ["How are you?", "Who are you?", "1+1=?"]
