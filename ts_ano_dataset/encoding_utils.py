...
def minmax_scale_encoding(timeseries: np.ndarray):
    ...
    return (
        scaled_timeseries[:, np.newaxis],
        prompt,
        {"offset": float(-mean), "scale_factor": float(scale_factor)},
    )


def minmax_encoding(timeseries: np.ndarray):
    # Scalar
    mean = np.mean(timeseries)
    scaled_timeseries = timeseries
    scale_factor = 1.0
    if np.any(np.abs(scaled_timeseries) >= 1.0):
        # Scale
        scale_factor = np.max(np.abs(scaled_timeseries)) / 1.0
        scaled_timeseries /= scale_factor
    prompt = f"[Scaled by: {scale_factor:.4f}]"
    scaled_timeseries = [round(num, 4) for num in scaled_timeseries]
    return scaled_timeseries, prompt, {"scale_factor": float(scale_factor)}
...
def sp_encoding(timeseries: np.ndarray):
    ...
    result_timeseries = np.stack(
        [scaled_timeseries, np.ones_like(scaled_timeseries)], axis=-1
    ).reshape(-1, 1)
...
def zscore_encoding(timeseries: np.ndarray):
    # Scalar
    mean = np.mean(timeseries)
    std = np.std(timeseries)
    scaled_timeseries = (timeseries - mean) / (std + 1e-5)
    result_timeseries = [round(num, 4) for num in scaled_timeseries]
    prompt = f"MEAN: {mean:.3f} STD: {std:.3f}"
    return result_timeseries, prompt, {"mean": round(mean, 3), "std": round(std, 3)}
...
def timeseries_encoding(timeseries: np.ndarray, method: str):
    if method == "minmax_scale":
        ...
        return minmax_encoding(timeseries)
    elif method == "z-score":
        return zscore_encoding(timeseries)
    else:
        raise NotImplementedError(
            f"Timeseries encoding method: {method} not implemented!"
        )
...
def timeseries_prompt(prompt: str, timeseries: np.ndarray):
    ...
    for i in range(len(timeseries)):
        result += (
            f"<ts>{[[round(k, 3) for k in j] for j in list(timeseries[i])]}<ts/>"
            + prompt_list[i + 1]
        )
...
def eval_prompt_to_encoding(
    prompt: str, timeseries: list, method: str
    ...
    padded_time_series_attributes = [
        np.pad(
            arr,
            ((0, 0), (0, max_length - arr.shape[1]), (0, 0)),
            mode="constant",
            constant_values=0,
        )
